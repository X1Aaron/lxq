...
echo
echo "Installing $c"
lxc exec $c -- snap install rocketchat-server
echo
echo "Downloading .conf file..."
wget --quiet -nc https://raw.githubusercontent.com/aaronstuder/lxq/master/apps/$c/$c.conf
sed -i "s/<<domain_name>>/$domain_name/g" $c.conf
sed -i "s/<<IP>>/$IP/g" $c.conf
echo
echo "Pushing .conf file to container..."
lxc file push $c.conf nginx/etc/nginx/conf.d/
